...
public class TransformResourceReloadListener implements SimpleSynchronousResourceReloadListener {
    ...
    public Identifier getFabricId() {
        return Identifier.of(NewFab.MOD_ID, "backslot_transforms");
    }

    @Override
    public void reload(ResourceManager manager) {
        transforms.clear();
        NewFab.LOGGER.info("Reloading transform data...");
        ...
        manager.findResources("backslot_transforms", path -> path.getPath().endsWith(".json")).keySet().forEach(id -> {
            try (InputStream stream = manager.getResource(id).get().getInputStream()) {
                ...
                result.resultOrPartial(NewFab.LOGGER::error).ifPresent(data -> {
                    NewFab.LOGGER.info("Loaded transform for: {}", data.item());
                ...
                NewFab.LOGGER.error("Failed to load transform for {}: {}", id, e.getMessage());
            }
        });

        NewFab.LOGGER.info("Loaded transforms: {}", transforms);
    }
